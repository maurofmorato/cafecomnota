-- ============================================================
-- Café com nota
-- Correção 1.0.7 - carga inicial com tipo_cafe e torra válidos
--
-- Ajustes:
-- - tipo_cafe usa valores técnicos: grao, moido
-- - torra usa valores técnicos seguros: media, escura
-- - não cria ranking
-- - não cria avaliação
-- - não inventa preço
-- ============================================================

-- Garante colunas novas usadas pelo catálogo.
...
-- Conferência opcional das constraints:
select
    conname,
    pg_get_constraintdef(oid) as definicao
from pg_constraint
where conrelid = 'public.cafes'::regclass
  and conname in ('cafes_tipo_cafe_chk', 'cafes_torra_chk')
order by conname;
...
values
    ...
    ('Coffee++ Frutado', 'Coffee++', 'grao', 'media', 'Especial'),
    ('Coffee++ Chocolate', 'Coffee++', 'grao', 'media', 'Especial'),
    ('Unique Cafés Frutado', 'Unique Cafés', 'grao', 'media', 'Especial'),
...
select
    count(*) as total_catalogo_inicial
from public.cafes
where origem_dado = 'catalogo_inicial';
